...
from ceo.models import EmergenctContact, Employees, ExcuseRequests, LeaveRequests
...
def settings(request):
    ...
    if request.method == "POST":
        ...
        if pic == "New default that isn't None":
            print("if worked")
            Employees.objects.filter(id=request.user.employee.id).update(
                name=name, phone=phone, email=email, whatsapp_number=whatsapp
            )
            EmergenctContact.objects.filter(employee=userdetails).update(
                emergency_number=emgnumber,
                primarycontact_name=emgname,
...
def leaves(request):
    ...
    context = {"user": user_name, "leavelist": leavelist}
