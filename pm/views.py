...
def index(request):
    ...
    advancepaid = (
        Enquiry.objects.filter(status="Advance Paid")
        .exclude(type="Graphic Designing")
        .count()
    )
...
def enquiry(request):
    ...
    for i in enquirylistdata:
        print(i.type)
    context = {"is_enquiry": True, "pm": pm, "enquirylistdata": enquirylistdata}
...
def dailyprogress(request):
    ...
    projectlists = (
        DailyProgress.objects.filter(date=today)
        .values(
        ...
        .order_by("project")
        .distinct()
        .exclude(project__enquiry__type="Graphic Designing")
    )
    context = {
...
def membersearch(request):
    ...
    request.POST["leaderid"]
    employee_prjt = request.POST["memberObj"]
    request.POST["projectid"]
...
def reason(request):
    ...
    LeaveRequests.objects.filter(id=id).update(
        rejected_reason=rejectedreason, status="Rejected"
    )
    return JsonResponse({"value": "msg"})
...
def qcrework(request):
    ...
    projectcount.save()
    status = ProjectStatus.objects.filter(project=projectidd).update(
        status="Rework", completion=94
    )
    return JsonResponse({"value": "msg"})
