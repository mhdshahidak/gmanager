...
def employeeHome(request):
    ...
    print(meetingcount)
    reworklist1 = Reworks.objects.filter(
        project__member__team=employeedata, status="Not Seen"
    ).count()
    reworklist2 = Reworks.objects.filter(
        project__member__lead=employeedata, status="Not Seen"
    ).count()
...
def empRework(request):
    ...
    reworklist = Reworks.objects.filter(
        project__member__team=employeedata, status="Not Seen"
    ) | Reworks.objects.filter(project__member__lead=employeedata, status="Not Seen")

    context = {"is_rework": True, "emp": emp, "reworklist": reworklist}
...
def empDailyProgress(request, id):
    ...
    if request.method == "POST":
        ...
        if fileupload != None:
            ...
            ProjectStatus.objects.filter(project=project_obj).update(
...
def empProgressReport(request, id):
    ...
    members = ProjectMembers.objects.filter(project=projectdetails).values(
        "team__name", "team__id"
    )
    daily_report = DailyProgress.objects.filter(project=projectdetails).values(
...
def empTask(request):
    ...
    print(taskdetails)
    context = {"is_task": True, "emp": emp, "taskdetails": taskdetails}
...
def empHomework(request):
    emp = request.user.employee
    context = {"is_homework": True, "emp": emp}
...
def leaveApplication(request):
    emp = request.user.employee
    context = {"is_attendance": True, "emp": emp}
...
def allEmployees(request):
    ...
    context = {"is_employee": True, "all_emp": all_emp, "emp": emp}
...
def departmentwiseEmployee(request, id):
    ...
    context = {"emp": emp, "category": category, "employees": employees}
...
def empTimeline(request):
    ...
    context = {"emp": emp, "is_timeline": True, "employee_timeline": employee_timeline}
